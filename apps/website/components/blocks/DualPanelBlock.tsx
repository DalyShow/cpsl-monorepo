...
export interface DualPanelBlockProps {
  ...
  rightPanelSecondary?: SanityPanel;
  headingLevel?: "h1" | "h2";
}
 ...
 */
export function DualPanelBlock({ leftPanel, rightPanel, rightPanelSecondary, headingLevel }: DualPanelBlockProps) {
  const left = mapPanel(leftPanel);
  const right = mapPanel(rightPanel);
  if (!left || !right) return null;
  const rightSecondary = mapPanel(rightPanelSecondary) ?? undefined;
  return <DualPanel left={left} right={right} rightSecondary={rightSecondary} headingLevel={headingLevel} />;
}
